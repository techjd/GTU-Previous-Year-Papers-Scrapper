...
def btn_clicked():
    if not is_connected():
        ...
        if is_not_blank(entry1.get()) and is_not_blank(entry3.get()) and is_not_blank(entry0.get()):
...
entry1 = Entry(
    bd=0,
    bg="#e9e4e4",
    highlightthickness=0, font=('Inter', 22),
)
...
entry2.config(bg="#E9E4E4")
entry2.config(font=('Inter', 22))
entry2.config(relief="flat")
entry2['menu'].configure(font=('Inter', 20))
entry2['menu'].configure(relief="flat")
